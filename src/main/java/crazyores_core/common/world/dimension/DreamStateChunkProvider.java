...
//		COLogger.write(Level.INFO, "Replacing block for biome.");
		...
		ChunkProviderEvent.ReplaceBiomeBlocks event = new ChunkProviderEvent.ReplaceBiomeBlocks(this, chunkX, chunkY, blocksInChunk, bytesInChunk, biomes, this.worldObj);
		MinecraftForge.EVENT_BUS.post(event);
		if (event.getResult() == Result.DENY) return;
		
		double d0 = 0.03125D;
		this.stoneNoise = this.noisePerl.func_151599_a(this.stoneNoise, (double)(chunkX * 16), (double)(chunkY * 16), 16, 16, d0 * 2.0D, d0 * 2.0D, 1.0D);
		
		for (int k = 0; k < 16; ++k) {
			for (int l = 0; l < 16; ++l) {
				DreamStateBiomes biomegenbase = (DreamStateBiomes) biomes[l + k * 16];
				biomegenbase.genTerrainBlocks(this.worldObj, this.rand, blocksInChunk, bytesInChunk, chunkX * 16 + k, chunkY * 16 + l, this.stoneNoise[l + k * 16]);
			}
		}
	}
	...
	public Chunk provideChunk(int chunkX, int chunkZ) {
		Block[] blocksInChunk = new Block[65536];
		byte[] bytesInChunk = new byte[65536];
		this.rand.setSeed((long)chunkX * 341873127512L + (long)chunkZ * 143897987541L);
		
		//TODO: Customize this
		this.generateTerrain(chunkX, chunkZ, blocksInChunk);
		this.biomesForGeneration = this.worldObj.getWorldChunkManager().loadBlockGeneratorData(this.biomesForGeneration, chunkX * 16, chunkZ * 16, 16, 16);
		
		this.replaceBlocksForBiome(chunkX, chunkZ, blocksInChunk, bytesInChunk, this.biomesForGeneration);
		
		//TODO: Customize these
//		this.caveGenerator.func_151539_a(this, this.worldObj, chunkX, chunkZ, blocksInChunk);
//		this.ravineGenerator.func_151539_a(this, this.worldObj, chunkX, chunkZ, blocksInChunk);
//		if (this.mapFeaturesEnabled) {
//			this.scatteredFeatureGenerator.func_151539_a(this, this.worldObj, chunkX, chunkZ, blocksInChunk);
//		}
		
		Chunk chunk = new Chunk(this.worldObj, blocksInChunk, bytesInChunk, chunkX, chunkZ);
		byte[] biomeIDs = chunk.getBiomeArray();
		for (int k = 0; k < biomeIDs.length; ++k){
			biomeIDs[k] = (byte)this.biomesForGeneration[k].biomeID;
		}
	...
	private void generateNoise(int x, int y, int z) {
		...
		this.noise5 = this.noiseGen6.generateNoiseOctaves(this.noise5, x, z, 5, 5, 200.0D, 200.0D, 0.5D);
	...
	/**
	 * Populates chunk with ores, map features, dungeons, etc.
	 */
	@Override
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
//		//TODO: CUSTOMIZE ALL THIS STUFF!
//		
...
////			(new WorldGenLakes(BlockList.dream_state_water)).generate(this.worldObj, this.rand, k1, l1, i2);
//			(new DreamStateWorldGenLakes(Blocks.water)).generate(this.worldObj, this.rand, k1, l1, i2);
//		}
//
//		//Add Lava Pools ??
...
////				(new WorldGenLakes(BlockList.dream_state_lava)).generate(this.worldObj, this.rand, k1, l1, i2);
//				(new DreamStateWorldGenLakes(Blocks.lava)).generate(this.worldObj, this.rand, k1, l1, i2);
//			}
//		}
//
//		//Add Dungeons ??
...
////					this.worldObj.setBlock(k1 + k, i2 - 1, l1 + l, BlockList.dream_state_ice, 0, 2);
//					this.worldObj.setBlock(k1 + k, i2 - 1, l1 + l, Blocks.ice, 0, 2);
//				}
//
//				if (this.worldObj.func_147478_e(k1 + k, i2, l1 + l, true)) {
////					this.worldObj.setBlock(k1 + k, i2, l1 + l, BlockList.dream_state_snow_layer, 0, 2);
